...
if six.PY2:
    input_item_separator = b'\x00'
    protocol = 0
else:
    # there will be some cases using python3 pickled data uses the \x00 binary
    # this is the case with datetimes
    # this is why we use a different separator for python3
    # this is not a bullet proof separator and may one day hit someone.
    # we provide a way to specify your own data separator to avoid this
    # problem
    input_item_separator = b'\x00\x01\x37\x42\x00'
...
def get_input_item_str_flow(
        source_file, separator=input_item_separator, chunk_size=32768
):
...
def get_input_item_flow(
    ...
    """
    for line in get_input_item_str_flow(
            source_file, separator=separator, chunk_size=chunk_size
    ):
        yield pickle.loads(line)


def chain_splitters(
        input_flow, splitters_params, separator=input_item_separator
):
...
def get_splitter(
        input_flow=None, max_items=200,
        split_attribute=None, force_split=False
):
    ...
    * use the `InputSplitterByAttribute` to guarantee that two items from the
    ...
    """
    if split_attribute:
        splitter = InputSplitterByAttribute(
            input_flow, max_items, split_attribute, force_split
        )
    else:
...
class InputSplitterSingle(object):
    ...
    def push(self, input_item):
        ...
        self.current_bucket.write(
            pickle.dumps(input_item, protocol=protocol) + input_item_separator
        )
        ...
        if (
                not self.max_items == 0 and
                self.current_bucket_size >= self.max_items
        ):
...
class InputSplitterByAttribute(object):
    ...
    def __init__(self, input_flow, max_items, split_attribute, force_split):
        ...
        # dictionary indexed by reference value contain the number found of
        # reference value
        self.reference_size = dict()

        # dictionary indexed by line number contain the reference value
        self.line_reference = dict()

        # dictionary indexed by filename contain the file object
        self.bucket_files = dict()

        # dictionary indexed by filename contain the number of line from the
        # file
        self.bucket_size = dict()

        # list of existing/available bucket file names
        self.bucket_filenames = list()

        # dictionary indexed by reference value contain bucket filename
        self.bucket_reference_value = dict()

    def __close_buckets(self):
        """Close all existing bucket files
        """
        for data_file in list(self.bucket_files.values()):
    ...
    def __choose_bucket(self, line_number):
        ...
        # Value counts were already checked in the __check_references
        # function.
        # Unfortunately, some items can be pushed directly through the
        # store_input_item function. As long as the latter can be called
        ...
        if not self.force_split:
            ...
            for bucket_filename in self.bucket_filenames:
                ...
                if self.max_items == 0 or (
                        bucket_size + ref_value_count
                ) <= self.max_items:
                    # let's reuse this bucket
                    self.bucket_reference_value[
                        reference_value
                    ] = bucket_filename
                    ...
                    # close it to avoid the infamous
    ...
    def __get_reference_value(self, input_item):
        ...
        if not hasattr(input_item, self.split_attribute):
            raise InputSplitterError(
                (
                    'Input item does not have a '
                    '%s attribute' % self.split_attribute
                )
            )
    ...
    def __check_references(self):
        ...
        for ref_value in list(self.reference_size.keys()):
    ...
    def __prepare(self, input_flow):
        ...
        for line_number, input_item in enumerate(input_flow):
            ...
            self.store_input_item(
                file_data, pickle.dumps(
                    input_item, protocol=protocol
                )
            )
    ...
    @staticmethod
    def store_input_item(
            source_file, input_item_bin, separator=input_item_separator
    ):
    ...
    def split(self):
        ...
        for line_number, input_item_str in enumerate(
                get_input_item_str_flow(data_file)
        ):
